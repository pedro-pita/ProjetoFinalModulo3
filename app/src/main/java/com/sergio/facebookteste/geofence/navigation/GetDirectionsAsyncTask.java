...
public class GetDirectionsAsyncTask extends AsyncTask<Map<String, String>, Object, Document> {
    ...
    public void onPostExecute(Document result) {
        ...
        if (exception == null) {
            activity.handleGetDirectionsResult(md.getDirection(result),
                    distanceText, durationText);
            //activity.handleGetDurationValue(String.valueOf(md.getDurationValue(result)),String.valueOf(md.getDurationText(result)));
            // activity.handleGetDistanceValue(String.valueOf(md.getDistanceValue(result)),String.valueOf(md.getDistanceText(result)));

            // Log.i("DurationValue", "DurationValue --"+md.getDurationValue(result));
        } else {
            processException();
        }
    }
    ...
    protected Document doInBackground(Map<String, String>... params) {
        ...
        try {
            LatLng fromPosition =
                    new LatLng(Double.valueOf(paramMap.get(USER_CURRENT_LAT)),
                            Double.valueOf(paramMap.get(USER_CURRENT_LONG)));
            LatLng toPosition =
                    new LatLng(Double.valueOf(paramMap.get(DESTINATION_LAT)),
    ...
    private void processException() {
        Toast.makeText(activity, "Error retriving data",
                3000).show();
    }
